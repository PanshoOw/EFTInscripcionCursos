...
public class S3RepositoryImpl implements S3Repository{
    ...
    public List<Asset> listObjectsInBucket(String bucket) {
        List<Asset> items = 
            s3Client.listObjectsV2(bucket).getObjectSummaries().stream()
                ...
                .map(key -> (Asset) mapS3ObjectToAsset(bucket, key))
                .collect(Collectors.toList());

        log.info("Listando objetos en el bucket: {}", bucket);
        return items;
    }

    private Asset mapS3ObjectToAsset(String bucket, String key) {
    ...
    public void moveObject(String bucketName, String fileKey, String destinationFileKey) {
        CopyObjectRequest copyObjectRequest = new CopyObjectRequest(bucketName, destinationFileKey, bucketName, fileKey);
